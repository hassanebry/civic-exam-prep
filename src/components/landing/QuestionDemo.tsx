...
export function QuestionDemo() {
  ...
  function getOptionClasses(index: number): string {
    const base =
      "flex w-full items-center gap-3 rounded-lg border px-4 py-3.5 text-left text-sm font-medium transition-all";
  ...
  return (
    <section id="demo-question" className="scroll-mt-20 px-6 py-16">
      <div className="mx-auto max-w-2xl">
        <h2 className="text-center font-serif text-2xl text-foreground">
          Testez-vous maintenant
        </h2>
        <p className="mt-2 text-center text-sm text-muted">
          Exemple de question officielle — niveau CSP
        </p>

        <div className="mt-8 rounded-xl border border-border bg-card p-6 shadow-[var(--shadow-sm)] sm:p-8">
          {/* Question */}
          ...
          <div className="mt-6 flex flex-col gap-2.5">
            {OPTIONS.map((opt, i) => (
              ...
              >
                <span className={getBadgeClasses(i)}>
          ...
          {answered && (
            <div className="mt-6 animate-[fadeIn_0.3s_ease-out]">
              <div className="flex gap-2.5 rounded-lg bg-[#EEF2FF] px-4 py-3">
              ...
              <Link
                href="/login"
                className="mt-5 inline-flex h-12 w-full items-center justify-center rounded-lg bg-primary text-sm font-semibold text-white transition-opacity hover:opacity-90"
              >
                Continuez l&apos;entraînement — 655 questions restantes
              </Link>
            </div>
          )}
        </div>
